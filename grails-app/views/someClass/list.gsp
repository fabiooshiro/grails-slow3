	...
	<body>
		...
		<div id="list-someClass" class="content scaffold-list" role="main">
			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="name" title="${message(code: 'someClass.name.label', default: 'Name')}" />
				...
				<g:each in="${someClassInstanceList}" status="i" var="someClassInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${someClassInstance.id}">${fieldValue(bean: someClassInstance, field: "name")}</g:link></td>
			...
			<div class="pagination">
				<g:paginate total="${someClassInstanceTotal}" />
			</div>
		</div>
	</body>
</html>
